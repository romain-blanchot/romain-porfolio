...
export const projectFormSchema = (t: ReturnType<typeof useI18n>) => 
    z.object({
        name: z.string().min(1, { error: t("project-form.required.name") }).max(255),
        surname: z.string().min(1, { error: t("project-form.required.surname") }).max(255),
        email: z.string().email({ error: t("project-form.required.email") }).max(255),
        company: z.string().min(1, { error: t("project-form.required.company") }).max(255),
        sector: z.string().min(1, { error: t("project-form.required.sector") }).max(255),
        budget: z.string().min(1, { error: t("project-form.required.budget") }).max(255),
        timeline: z.string().min(1, { error: t("project-form.required.timeline") }).max(255),
        mainObjective: z.string().min(1, { error: t("project-form.required.mainObjective") }).max(2000),
        projectIdea: z.string().min(1, { error: t("project-form.required.projectIdea") }).max(2000),
        targetAudience: z.string().min(1, { error: t("project-form.required.targetAudience") }).max(2000),
        expectedResults: z.string().min(1, { error: t("project-form.required.expectedResults") }).max(2000),
